...
export type Snapshot = { legacyAttendance?: { studentId: string; day: string; value: string; color: string }[]; students: { id: string; name: string; phone: string; sourceStudentId?: string; subject?: string; instruments?: string[] }[]; accounts: Account[]; attendance: Attendance[]; invoices: Invoice[]; payments: Payment[]; notices: Notice[]; devices: Device[]; day: string; configured: boolean };
