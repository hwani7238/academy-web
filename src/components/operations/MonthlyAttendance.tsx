...
const GROUPS = ['성인 피아노', '어린이 피아노(1관)', '어린이 피아노(2관)', '앙상블', '보컬', '드럼', '우쿨렐레', '기타', '미디'];
function groupName(student: Snapshot['students'][number]) {
...
const compareGroups = (a: string, b: string) => rank(a) - rank(b) || korean.compare(a, b);
export function MonthlyAttendance({ data, day, busy, save }: { data: Snapshot; day: string; busy: boolean; save: (v: Record<string, unknown>) => Promise<unknown> }) {
  ...
  const filteredStudents = data.students.filter(s => s.name.includes(search.trim()) && (!subject || groupName(s) === subject)).sort((a, b) => compareGroups(groupName(a), groupName(b)) || korean.compare(a.name.split(' · ')[0], b.name.split(' · ')[0]) || korean.compare(a.id, b.id));
  ...
  const legacy = new Map((data.legacyAttendance || []).map(a => [`${a.studentId}_${a.day}`, a]));
