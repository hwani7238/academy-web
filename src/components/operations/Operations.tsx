...
import { compareStudents, displayCourseName } from '@/lib/operations/student-order';
...
export function Operations({ demo = false }: { demo?: boolean }) {
  ...
  return <div className="whee-ops operations">
    ...
    {tab === 'students' && <><div className="section-head"><div><h2>학생별 수강 설정</h2><p>현재 남은 횟수는 처음 등록할 때만 입력합니다.</p></div><label>학생 찾기<input placeholder="이름 또는 전화번호" value={search} onChange={e => setSearch(e.target.value)} /></label></div><div className="table-wrap"><table><thead><tr><th>학생</th><th>수강권</th><th>수강료</th><th>남은 횟수</th><th>상태</th><th>관리</th></tr></thead><tbody>{students.map(s => { const a = data.accounts.find(a => a.id === s.id); return <tr key={s.id}><td><strong>{displayCourseName(s.name)}</strong><small>{s.phone}</small></td><td>{a ? `${a.planUnits}회` : '설정 전'}</td><td>{a ? won(a.planAmount) : '—'}</td><td><span className={a && a.remaining <= 0 ? 'pill amber' : ''}>{a ? `${a.remaining}회` : '—'}</span></td><td>{!a ? '설정 필요' : a.active ? '수강 중' : '중지'}</td><td><button disabled={busy} onClick={() => setPanel({ type: 'account', id: s.id })}>{a ? '설정 수정' : '수강 등록'}</button>{a && !a.openInvoiceId && <button disabled={busy} className="quiet" onClick={() => click({ action: 'invoice', studentId: s.id })}>청구 만들기</button>}</td></tr>; })}</tbody></table></div>{!students.length && <div className="empty">학생이 없습니다. 기존 학생 관리에서 먼저 등록해주세요.</div>}</>}
