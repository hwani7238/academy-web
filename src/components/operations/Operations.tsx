...
export function Operations({ demo = false }: { demo?: boolean }) {
  ...
  return <div className="whee-ops operations">
    ...
    <header className="ops-header"><div><p className="brand">WHEE MUSIC</p><h1>{tab === 'home' ? '관리 홈' : '출석·수납 관리'}</h1></div><div className="header-actions"><span className="subtle">{demo ? '원장님 화면 체험' : user?.email}</span><a href={demo ? '/check-in/demo' : '/check-in'} target="_blank" rel="noreferrer">출석 화면 ↗</a></div></header>
