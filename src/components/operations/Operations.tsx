...
export function Operations({ demo = false }: { demo?: boolean }) {
  ...
  const inactiveStudents=[...new Map(data.students.filter(s=>enrollmentState(s.lifecycle,todayDate)!=='active').map(s=>[s.sourceStudentId||s.id,s])).values()].sort(compareStudents);
  ...
  const tabs = [['students', '총 등록 현황'], ['monthly', '월별 출석표'], ['today', '출석 기록'], ['billing', '청구·수납'], ['notices', '알림 내역'], ['devices', '출석 기기'], ...(!demo ? [['imports', '기존 장부']] : []), ['inactive', '퇴원 및 휴원']];
  return <div className="whee-ops operations">
    ...
    {tab === 'inactive' && <><div className="section-head"><div><h2>퇴원 및 휴원</h2><p>휴원 종료일 다음 날 자동으로 복귀합니다. 이전 기록과 잔여 횟수는 보관됩니다.</p></div></div><div className="table-wrap"><table><thead><tr><th>학생</th><th>상태</th><th>휴원 종료일</th><th>비고</th><th>관리</th></tr></thead><tbody>{inactiveStudents.map(s=><tr key={s.sourceStudentId||s.id}><td><strong>{s.name.split(' · ')[0]}</strong><small>{s.phone}</small></td><td>{enrollmentState(s.lifecycle,todayDate)==='paused'?'휴원':'퇴원'}</td><td>{s.lifecycle?.until||'—'}</td><td>{s.lifecycle?.note||'—'}</td><td><button disabled={busy} onClick={()=>click({action:'changeLifecycle',sourceStudentId:s.sourceStudentId||s.id,status:'active',expectedUpdatedAt:s.lifecycle?.updatedAt||''})}>{enrollmentState(s.lifecycle,todayDate)==='paused'?'지금 복귀':'재등록'}</button></td></tr>)}</tbody></table></div>{!inactiveStudents.length&&<p className="empty">퇴원·휴원한 학생이 없습니다.</p>}</>}
    ...
    {panel && <div className="panel-backdrop"><section className="edit-panel" role="dialog" aria-modal="true" aria-labelledby="edit-title"><div className="section-head"><h2 id="edit-title">{panel.type === 'account' ? `${student?.name} · 수강 설정` : panel.type === 'adjust' ? `${panel.row.name} · 출석 수정` : `${panel.row.name} · 수납 기록`}</h2><button disabled={busy} onClick={() => setPanel(null)} aria-label="닫기">닫기</button></div>
      {panel.type === 'account' && <form onSubmit={e => submit(e, 'configure', { studentId: panel.id, ...(student?.sourceStudentId ? { sourceStudentId: student.sourceStudentId, subject: student.subject } : {}) })} key={panel.id}><label>수강권 횟수<input name="planUnits" type="number" min="1" max="200" defaultValue={account?.planUnits || 8} required /></label><label>수강료 (원)<input name="planAmount" type="number" min="1" max="100000000" defaultValue={account?.planAmount || ''} required /></label>{!account && <label>현재 남은 횟수<input name="remaining" type="number" min="-1000" max="1000" defaultValue="0" required /><small>기존 장부 기준으로 입력하세요. 미납 상태로 초과 수업한 횟수는 음수로 입력할 수 있어요.</small></label>}<label>알림 받을 보호자 전화번호<input name="phone" type="tel" defaultValue={account?.phone || student?.phone} required /></label><label>출석에 사용할 전화번호 또는 뒷번호<input name="phones" defaultValue={account?.checkinSuffixes.join(', ') || student?.phone} required /><small>본인·보호자 번호를 쉼표로 구분하세요. 예: 1234, 5678</small></label><label className="check"><input name="active" type="checkbox" defaultChecked={account?.active ?? true} />출석 허용</label><label className="check"><input name="autoBilling" type="checkbox" defaultChecked={account?.autoBilling ?? false} />수강 횟수 소진 시 결제 안내도 자동 요청</label><p className="subtle">자동 안내를 켜지 않아도 청구 대상은 자동으로 만들어집니다.</p><button className="primary" disabled={busy}>수강 설정 저장</button></form>}
      {panel.type === 'adjust' && <form onSubmit={e => submit(e, 'adjust', { attendanceId: panel.row.id })}><label>차감 횟수<input name="units" type="number" min="0" max="10" defaultValue={panel.row.units} required /><small>2회 연속 수업은 2, 차감 취소는 0을 입력하세요.</small></label><label>변경 사유·비고 (선택)<textarea name="note" defaultValue={panel.row.note} maxLength={500} /></label><button className="primary" disabled={busy}>변경 기록 저장</button></form>}
