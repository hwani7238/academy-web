...
export function Operations({ demo = false }: { demo?: boolean }) {
  ...
  return <div className="whee-ops operations">
    ...
    <main className="ops-main"><div className="section-head"><div><h2>조회 날짜 · {day}</h2><p>출석 기록을 보는 날짜입니다. 아이폰 출석은 한국 시간의 오늘 날짜로 자동 저장됩니다.</p></div><div className="header-actions"><label>출석 조회일<input type="date" value={day} onChange={e => { if (e.target.value) setDay(e.target.value); }} /></label><button onClick={() => setDay(seoulDay())}>오늘</button></div></div><div className="summary-grid">
      ...
      <div><span>총 등록 현황</span><strong>{data.accounts.length}<small>/ {data.students.length}건</small></strong>{unconfigured > 0 && <button className="text-button" onClick={() => setTab('students')}>{unconfigured}건 설정 필요 →</button>}</div>
    </div>
    ...
    {tab === 'students' && <><div className="section-head"><div><h2>총 등록 현황</h2><p>현재 남은 횟수는 처음 등록할 때만 입력합니다.</p></div><button className="primary" disabled={busy} onClick={()=>setRegistering(true)}>＋ 신규 등록</button><label>과목<select value={subject} onChange={e => setSubject(e.target.value)}><option value="">전체 과목</option>{subjects.map(s => <option key={s} value={s}>{s}</option>)}</select></label><label>학생 찾기<input placeholder="이름 또는 전화번호" value={search} onChange={e => setSearch(e.target.value)} /></label></div><div className="table-wrap"><table><thead><tr><th>학생</th><th>수강권</th><th>수강료</th><th>남은 횟수</th><th>상태</th><th>관리</th></tr></thead><tbody>{students.map(s => { const a = data.accounts.find(a => a.id === s.id); return <tr key={s.id}><td><strong>{displayCourseName(s.name)}</strong><small>{s.phone}</small></td><td>{a ? `${a.planUnits}회` : '설정 전'}</td><td>{a ? won(a.planAmount) : '—'}</td><td><span className={a && a.remaining <= 0 ? 'pill amber' : ''}>{a ? `${a.remaining}회` : '—'}</span></td><td>{!a ? '설정 필요' : a.active ? '수강 중' : '중지'}</td><td><button disabled={busy} onClick={() => setPanel({ type: 'account', id: s.id })}>{a ? '설정 수정' : '수강 등록'}</button>{a && !a.openInvoiceId && <button disabled={busy} className="quiet" onClick={() => click({ action: 'invoice', studentId: s.id })}>청구 만들기</button>}</td></tr>; })}</tbody></table></div>{!students.length && <div className="empty">선택한 과목과 검색어에 해당하는 학생이 없습니다.</div>}</>}
