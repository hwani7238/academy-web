...
export function CheckIn({ demo }: { demo?: Demo }) {
  ...
  return <div className={`whee-ops kiosk ${demo ? 'embedded' : ''}`}>
    ...
    </form> : success ? <div className="kiosk-success" role="status"><span aria-hidden="true">✓</span><h2>{message}</h2><p>잠시 후 처음 화면으로 돌아갑니다.</p></div> : matches.length ? <div className="matches"><p>아직 출석 전입니다. 본인 이름의 출석하기를 눌러주세요.</p>{matches.map(m => <button disabled={busy} key={m.id} onClick={() => void attend(m.id)}>{m.name}<span>출석하기 →</span></button>)}<button className="quiet" onClick={() => { setMatches([]); setDigits(''); setMessage(''); }}>번호 다시 입력</button></div> : <form onSubmit={e => { e.preventDefault(); void lookup(); }}>
      <p>번호 4자리 입력 → 이름 확인 → 본인 이름을 눌러 출석 완료</p><label className="sr-only" htmlFor={demo ? 'demo-digits' : 'digits'}>휴대폰 뒷번호</label><input id={demo ? 'demo-digits' : 'digits'} className="digit-display" inputMode="numeric" autoComplete="off" maxLength={4} value={digits} onChange={e => setDigits(e.target.value.replace(/[^0-9]/g, '').slice(0, 4))} placeholder="····" />
